...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    // Copy extra pixels for bottom and right parts.
    bool x_in_bounds = (threadIdx.x < 2 && index_x + BLOCK_SIZE < width);
    bool y_in_bounds = (threadIdx.y < 2 && index_y + BLOCK_SIZE < height);

    if (x_in_bounds)
        sh_block[offset_sh + BLOCK_SIZE] = image[offset_t + BLOCK_SIZE];
    if (y_in_bounds)
        sh_block[offset_sh + (BLOCK_SIZE * BLOCK_SIZE_SH)] = image[offset_t + (BLOCK_SIZE * width)];
    if (x_in_bounds && y_in_bounds)
        sh_block[offset_sh + (BLOCK_SIZE * (BLOCK_SIZE_SH + 1))] = image[offset_t + (BLOCK_SIZE * (width + 1))];
    ...
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        ...
        image_out[offset] = applyFilter(&sh_block[offset_sh],

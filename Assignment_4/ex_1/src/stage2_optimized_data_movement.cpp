...
void semi_discrete_step( double *state_init , double *state_forcing , double *state_out , double dt , int dir , double *flux , double *tend ) {
  ...
  #pragma acc parallel loop collapse(3)                   \
    copyout(state_out[0 : NUM_VARS*(nz+2*hs)*(nx+2*hs)])     \
    copyin(state_init[0 : NUM_VARS*(nz+2*hs)*(nx+2*hs)])    \
...
void set_halo_values_x( double *state ) {
  ...
  #pragma acc parallel loop collapse(3)                \
    copyin(state[0 : NUM_VARS*(nz+2*hs)*(nx+2*hs)])      \
    copyout(recvbuf_l[0 : hs*nz*NUM_VARS])                \
    copyout(recvbuf_r[0 : hs*nz*NUM_VARS])
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (s=0; s<hs; s++) {
  ...
  if (data_spec_int == DATA_SPEC_INJECTION) {
    if (myrank == 0) {
      #pragma acc parallel loop collapse(2)                \
        copyout(state[0 : NUM_VARS*(nz+2*hs)*(nx+2*hs)])      \
